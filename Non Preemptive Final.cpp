...
class processes
    {
        public:
            ...
            void add_after(int value,int prio,int time);
            void remove_process(int value);
            void search_element(int value);
            void display_processes();
            void reduce();
            int get_pos(int value,int prio,int time);
            processes()
            {
                start = NULL;  
            }
...
//---------------------------------------------------------------------------------------------------------------------    
void processes::add_after(int value,int prio,int time)
    {
    	int position;
		position=get_pos(value,prio,time);					//fetch location after comparing prio, burst and order of arrival
        if (start == NULL)
        {
...
//---------------------------------------------------------------------------------------------------------------------
int processes::get_pos(int value,int prio,int time)
    {
    	int position=0;
    	struct process *temp;
        if (start == NULL)
        ...
        if(procount==1)
        	...
        	if(procount==2)
        		{
				if( (start->next->at==time) && (start->next->prio < prio) )
        			return 1;
        		else
        			if( (start->next->at==time) && (start->next->prio == prio) && (start->next->remtime > value) )
        				return 1;
        			else
        				return 2;			
        		}
        		...
        		if(procount>=3)
					...
					if( (start->next->at==time) && (start->next->prio < prio) )
						...
						if( (start->next->at==time) && (start->next->prio == prio) && (start->next->remtime > value) )
							...
							{
								position = 2;
								while( (temp->prio > prio) && (temp->next!=NULL) )
									{
									position++;
									temp=temp->next;
									}
								if( (temp->prio > prio) && (temp->next==NULL) )
									{
										return position;
									}
								while( (temp->prio == prio) && (temp->next!=NULL) && (temp->remtime < value) )
									{
										temp = temp->next;
										position++;
									}
								if( (temp->prio == prio) && (temp->next!=NULL) && (temp->remtime == value) )
									{
										temp = temp->next;
										position++;
									}
		...
		cout<<"ERROR : FATAL ERROR (This should never happen";
		do{}while(!kbhit());
		return position;
    }
//---------------------------------------------------------------------------------------------------------------------
